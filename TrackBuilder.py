...
class TrackBuilder:
    ...
    def SetAttributes(self):
        ...
        self.mouse_binds = {"MOUSEBUTTONDOWN" : {1 : self.LeftMouseButtonDown,
                                                 4 : self.ZoomIn,
                                                 5 : self.ZoomOut},
                            "MOUSEBUTTONUP" : {1 : [self.AddPoint, self.LeftMouseButtonUp],
                                               3 : self.DeletePoint}}
    ...
    def ZoomIn(self):
        ...
        if(self.max_zoom_reached == False):
            self.screen_pos = ((1 - self.zoom_move_factor) * self.screen_pos[0] + self.zoom_move_factor * self.global_mouse_pos[0], (1 - self.zoom_move_factor) * self.screen_pos[1] + self.zoom_move_factor * self.global_mouse_pos[1])
    ...
    def ZoomOut(self):
        ...
        if(self.min_zoom_reached == False):
            self.screen_pos = ((1 + self.zoom_move_factor) * self.screen_pos[0] - self.zoom_move_factor * self.global_mouse_pos[0], (1 + self.zoom_move_factor) * self.screen_pos[1] - self.zoom_move_factor * self.global_mouse_pos[1])


    def AddPoint(self):
        if(self.n_points > 0 and self.global_mouse_pos == self.points[-1] or self.closed or self.point_hover_index > 0 or self.point_hover_index == 0 and self.n_points < 3):
            return
        ...
        if(self.drag_index != -1):
            if(self.drag_type == 0):
                if(self.edited_rails[self.drag_index] == True):
                    ...
                    self.right_rails[self.drag_index] = (self.right_rails[self.drag_index][0] + self.global_mouse_pos[0] - self.points[self.drag_index][0], self.right_rails[self.drag_index][1] + self.global_mouse_pos[1] - self.points[self.drag_index][1])
                ...
                self.points[self.drag_index] = ((self.global_mouse_pos[0] + self.right_rails[self.drag_index][0]) / 2, (self.global_mouse_pos[1] + self.right_rails[self.drag_index][1]) / 2)

                self.edited_rails[self.drag_index] = True

            elif(self.drag_type == 2):
                self.right_rails[self.drag_index] = self.global_mouse_pos
                self.points[self.drag_index] = ((self.global_mouse_pos[0] + self.left_rails[self.drag_index][0]) / 2, (self.global_mouse_pos[1] + self.left_rails[self.drag_index][1]) / 2)
    ...
    def CheckPointHover(self):
        ...
        if(self.n_points == 1):
            min_distance = ((self.global_mouse_pos[0] - self.points[0][0]) ** 2 + (self.global_mouse_pos[1] - self.points[0][1]) ** 2) ** 0.5
        ...
        for point_index, point in enumerate(self.points[1:], start=1):
            distance = ((self.global_mouse_pos[0] - point[0]) ** 2 + (self.global_mouse_pos[1] - point[1]) ** 2) ** 0.5
        ...
        rail_start_index = 0 if self.closed else 1
        rail_stop_index = self.n_points if self.closed else self.n_points - 1

        for point_index, point in enumerate(self.left_rails[rail_start_index:rail_stop_index], start=rail_start_index):
            distance = ((self.global_mouse_pos[0] - point[0]) ** 2 + (self.global_mouse_pos[1] - point[1]) ** 2) ** 0.5
        ...
        for point_index, point in enumerate(self.right_rails[rail_start_index:rail_stop_index], start=rail_start_index):
            distance = ((self.global_mouse_pos[0] - point[0]) ** 2 + (self.global_mouse_pos[1] - point[1]) ** 2) ** 0.5
    ...
    def CalculateRails(self):
        if(self.n_points >= 3):
            ...
            ab_p = self.track_width * np.concatenate((-ab[:, 1, None], ab[:, 0, None]), axis=-1) / ab_length

            bc = c - b
            bc_length = np.sqrt(np.sum(bc ** 2, axis=-1, keepdims=True))
            bc_p = self.track_width * np.concatenate((-bc[:, 1, None], bc[:, 0, None]), axis=-1) / bc_length

            abc_M = np.concatenate((ab[..., None], bc[..., None]), axis=-1)

            try:
                t_left = (np.linalg.inv(abc_M) @ (c + bc_p - (a + ab_p))[..., None])[..., 0, 0, None]
                t_right = (np.linalg.inv(abc_M) @ (c - bc_p - (a - ab_p))[..., None])[..., 0, 0, None]
            ...
            if(self.closed == False):
                ...
                for i, (edited_rail, left_rail, right_rail) in enumerate(zip(self.edited_rails[1:self.n_points-1], (a + ab_p + ab * t_left).tolist(), (a - ab_p + ab * t_right).tolist()), start=1):
                    if(edited_rail == True):
                ...
                for i, (edited_rail, left_rail, right_rail) in enumerate(zip(self.edited_rails, (a + ab_p + ab * t_left).tolist(), (a - ab_p + ab * t_right).tolist())):
                    if(edited_rail == True):
                        left_rails.append(self.left_rails[i])
                        right_rails.append(self.right_rails[i])
    ...
    def GlobalPointsToScreenSpace(self, points : List[Tuple[float, float]]) -> List[Tuple[float, float]]:
        offset_points = []

        for point in points:
            offset_point = ((point[0] - self.screen_pos[0]) * self.zoom + self.screen_pos[0], (point[1] - self.screen_pos[1]) * self.zoom + self.screen_pos[1])
            offset_points.append(offset_point)

        return offset_points
    
    def GlobalPointToScreenSpace(self, point : Tuple[float, float]) -> Tuple[float, float]:
        return self.GlobalPointsToScreenSpace([point])[0]
    

    def ScreenPointsToGlobalSpace(self, points : List[Tuple[float, float]]) -> List[Tuple[float, float]]:
        offset_points = []
    ...
    def ScreenPointToGlobalSpace(self, point : Tuple[float, float]) -> Tuple[float, float]:
        return self.ScreenPointsToGlobalSpace([point])[0]
    

    def DrawRoad(self):
        for i in range(self.n_points - 1):
            pg.draw.polygon(self.screen, self.track_color, self.GlobalPointsToScreenSpace([self.left_rails[i], self.left_rails[i+1], self.right_rails[i+1], self.right_rails[i]]))

        if(self.closed == True):
            pg.draw.polygon(self.screen, self.track_color, self.GlobalPointsToScreenSpace([self.left_rails[0], self.left_rails[-1], self.right_rails[-1], self.right_rails[0]]))

    
    def DrawRails(self):
        pg.draw.lines(self.screen, self.rails_color, self.closed, self.GlobalPointsToScreenSpace(self.left_rails), width=floor(self.track_width*self.zoom/4))
        pg.draw.lines(self.screen, self.rails_color, self.closed, self.GlobalPointsToScreenSpace(self.right_rails), width=floor(self.track_width*self.zoom/4))


    def DrawRailHandles(self):
        start_index = 0 if self.closed == True else 1
        stop_index = self.n_points if self.closed else self.n_points - 1

        for i, (l, r) in enumerate(zip(self.left_rails[start_index:stop_index], self.right_rails[start_index:stop_index]), start=start_index):
            ...
            if(i == self.point_hover_index):
                if(self.point_hover_type == 1):
                    pg.draw.circle(self.screen, self.rail_handle_hover_color, l_s, radius)
                    pg.draw.circle(self.screen, self.rail_handle_color, l_s, radius, floor(radius / 3))

                    pg.draw.circle(self.screen, self.track_color, r_s, radius)
                    pg.draw.circle(self.screen, self.rail_handle_color, r_s, radius, floor(radius / 3))

                elif(self.point_hover_type == 2):
                    pg.draw.circle(self.screen, self.track_color, l_s, radius)
                    pg.draw.circle(self.screen, self.rail_handle_color, l_s, radius, floor(radius / 3))

                    pg.draw.circle(self.screen, self.rail_handle_hover_color, r_s, radius)
                    pg.draw.circle(self.screen, self.rail_handle_color, r_s, radius, floor(radius / 3))
    ...
    def DrawWayPoints(self):
        for i, point in enumerate(self.points):
            ...
            if(self.point_hover_type == 0 and i == self.point_hover_index):
                pg.draw.circle(self.screen, self.way_point_hover_color, point_s, point_r)
            else:
                pg.draw.circle(self.screen, self.track_color, point_s, point_r)

            pg.draw.circle(self.screen, self.way_point_color, point_s, point_r, floor(point_r / 3))
    ...
    def DrawStartLine(self):
        if(self.closed == True):
            ...
            pg.draw.line(self.screen, self.start_line_color, left_side, right_side, width=floor(self.track_width*self.zoom/5))

        
    def Draw(self):
        self.screen.fill(self.background_color)
        self.DrawTrack()
        pg.draw.circle(self.screen, self.cursor_color, self.screen_mouse_pos, self.cursor_radius)
    ...
    def Save(self):
        ...
        T.save({'left_rails': left_rails, 'right_rails': right_rails, 'points': points}, f"{track_save_dir}//data.pt")
