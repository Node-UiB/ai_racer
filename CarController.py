...
class CarController:
    def __init__(self, dtype : T.dtype, device : str):
        ...
        self.key_binds = {"w" : {"state" : False, "bind" : self.Accelerate}, 
        ...
        self.negative_keybinds = {("w", "s") : {"state" : False, "bind" : self.IdleAcceleration},
    ...
    def Accelerate(self):
        self.acceleration = self.acceleration_mu * self.acceleration + (1 - self.acceleration_mu) * self.forward.clone()
    ...
    def Decelerate(self):
        self.acceleration = self.acceleration_mu * self.acceleration + (1 - self.acceleration_mu) * self.back.clone()


    def TurnRight(self):
        self.wheel_angle = self.wheel_angle_mu * self.wheel_angle + (1 - self.wheel_angle_mu) * self.right.clone()
    ...
    def TurnLeft(self):
        self.wheel_angle = self.wheel_angle_mu * self.wheel_angle + (1 - self.wheel_angle_mu) * self.left.clone()
    ...
    def GetActions(self) -> Tuple[T.Tensor, T.Tensor, bool, bool]:
        self.CheckKeys()
        self.Update()
