...
class Camera:
    def __init__(self, 
        ...
        self.local_screen_center = T.as_tensor([self.x_res / 2, self.y_res * 4 / 5], dtype=dtype, device=device)
    ...
    def Update(self, new_position : T.Tensor, new_angle : T.Tensor, new_pixel_density : Optional[float] = None):
        self.position = self.position_mu * self.position + (1 - self.position_mu) * new_position
        self.angle = self.angle_mu * self.angle + (1 - self.angle_mu) * new_angle

        if(new_pixel_density is not None):
            self.pixel_density = self.pixel_density_mu * self.pixel_density + (1 - self.pixel_density_mu) * new_pixel_density

        self.UpdateRotationMatrix()


    def GlobalToLocalSpace(self, positions : T.Tensor) -> T.Tensor:
        return (self.inv_rotation_matrix @ (positions - self.position[None])[..., None])[..., 0] * self.pixel_density + self.local_screen_center[None]


    def LocalToGlobalSpace(self, local_positions : T.Tensor) -> T.Tensor:
        return (self.rotation_matrix @ (local_positions - self.local_screen_center[None])[..., None])[..., 0] / self.pixel_density + self.position[None]