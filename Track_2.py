...
class Track:
    ...
    def Load(self, track_number: int) -> "Track":
        try:
            ...
            return Track(
                track_data["left_rails"],
                track_data["right_rails"],
                track_data["points"],
            )
            ...
            raise FileNotFoundError(
                f"Track number '{track_number}' not valid. Choose between {'-' if len(track_names) == 0 else 1}-{'-' if len(track_names) == 0 else len(track_names)}"
            )
